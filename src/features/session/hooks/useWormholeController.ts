...
export const useWormholeController = () => {
			...
			void (async () => {
				...
				const message =
					error instanceof Error
						? error.message
						: "Failed to handle signalling payload.";
	...
	const joinSession = async (rawCode: string): Promise<void> => {
		...
		joinTimeoutRef.current = window.setTimeout(() => {
			void cancelPendingJoin("Join timed out after 30 seconds. The room code may be invalid or the connection may be unstable.");
		}, JOIN_TIMEOUT_MS);
